...
**Current Reality in Ethiopian Cafes:**
- No individual tablets/phones per waiter
- 1 shared device (phone/tablet/computer) per cafe/branch
- Multiple waiters (2-5) use same device during shift
- Manager assigns tables to waiters manually

**Solution:** Shared waiter portal with table assignment workflow
- ✅ **Proven approach** (Square POS, busy restaurants worldwide)
- ✅ **Hardware agnostic** (works on 1 shared Android phone)
- ✅ **Accountability maintained** (manager assigns + audit trail)
- ✅ **Fast workflow** (no login/logout delays)

## 🎯 WORKFLOW SPECIFICATION

### 1. SHARED WALTER PORTAL (Primary Screen)
Screen Flow:
[Shared Device Opens]
↓
"Select Waiter" Dropdown
(Abebe, Mulu, Selam - manager pre-populates)
↓
Waiter taps name → Session starts (5 min idle timeout)
↓
"Assigned Tables" Screen (Table 3, 5, 7 → shows only HIS tables)
↓
Take orders → Payment → Kitchen

text

### 2. MANAGER TABLE ASSIGNMENT
Manager Dashboard:
Table Layout (visual grid):
Table 1: [Unassigned] Table 2: [Abebe] Table 3: [Mulu]
Table 4: [Unassigned] Table 5: [Selam] Table 6: [Abebe]

Drag waiter name → Table OR Dropdown assignment
Real-time sync → Waiter portal shows only assigned tables
Auto-unassign after order completion + payment

text

### 3. SECURITY & ACCOUNTABILITY
Quick Waiter Selection (NO PASSWORD)
POST /api/v1/waiter/session/start
{
"waiter_name": "Abebe",
"branch_id": 123
}
...
Auto-logout: 5 minutes idle

Manual logout button

Back to "Select Waiter" screen

Sensitive Actions Require PIN

Discounts > 15%: Manager 4-digit PIN

Voids: Manager PIN

Refunds: Manager approval

Complete Audit Trail
EVERY order records:

assigned_waiter_name: "Abebe"
...
### Add to `orders` table:
```sql
ALTER TABLE orders ADD COLUMN assigned_waiter_name VARCHAR(100);
ALTER TABLE orders ADD COLUMN waiter_session_id UUID;
ALTER TABLE orders ADD COLUMN table_assigned_at TIMESTAMP;

ALTER TABLE order_items ADD COLUMN assigned_waiter_name VARCHAR(100); -- Denormalized for fast reporting
New tables:
sql
-- Manager assigns tables to waiters
...
-- Waiter sessions (for audit trail)
CREATE TABLE waiter_sessions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  branch_id INTEGER NOT NULL,
  waiter_name VARCHAR(100) NOT NULL,
  device_id VARCHAR(100), -- Phone IMEI or browser fingerprint
...
Waiter Portal APIs
text
POST /api/v1/branches/:branchId/waiter/session/start
Body: { "waiter_name": "Abebe" }
Response: { session_id, assigned_tables: [] }

GET /api/v1/branches/:branchId/waiter/assigned-tables
Response: [
  { table_number: 3, status: "occupied", order_count: 1 },
  { table_number: 5, status: "free", order_count: 0 }
]

POST /api/v1/branches/:branchId/waiter/assigned-tables
Body: { table_number: 7 } // Claim table
Manager APIs
text
GET /api/v1/branches/:branchId/table-assignments
Response: [
  { table_number: 1, waiter_name: null, status: "free" },
  { table_number: 2, waiter_name: "Abebe", assigned_at: "..." }
]

POST /api/v1/branches/:branchId/table-assignments
Body: {
  "table_number": 3,
  "waiter_name": "Mulu"
}

DELETE /api/v1/branches/:branchId/table-assignments/:tableNumber
Order Creation (Enhanced)
text
POST /api/v1/branches/:branchId/orders
Body: {
  "table_number": 5,
  "waiter_name": "Abebe", // Auto-filled from session
  "items": [...]
}
→ Automatically records assigned_waiter_name in orders table
💻 IMPLEMENTATION CODE
1. Waiter Session Service
typescript
// src/services/waiter-session.service.ts
class WaiterSessionService {
  async startSession(branchId: number, waiterName: string, deviceId?: string) {
    const sessionId = generateUUID();
    
    await db.query(`
      INSERT INTO waiter_sessions (id, tenant_id, branch_id, waiter_name, device_id)
      VALUES ($1, $2, $3, $4, $5)
    `, [sessionId, tenantId, branchId, waiterName, deviceId]);
    
    return { session_id: sessionId, waiter_name: waiterName };
  }
}
...
const orderData = {
  ...input,
  assigned_waiter_name: req.waiter_session?.waiter_name, // From middleware
  waiter_session_id: req.waiter_session?.id
};
3. RBAC Middleware Update
typescript
// waiter.middleware.ts
app.post('/waiter/session/start', 
  authenticate(), 
  checkPermission('waiter.use_shared_portal'), // New permission
  startWaiterSession() // Sets req.waiter_session
);
📊 REPORTING (Bonus)
Waiter Performance Reports
text
Daily Sales by Waiter:
Abebe: 45 orders, 12,500 ETB, avg 277 ETB/order
Mulu:  32 orders,  8,900 ETB, avg 278 ETB/order
Selam: 28 orders,  7,600 ETB, avg 271 ETB/order
Query:
...
GROUP BY assigned_waiter_name;
🎨 UI MOCKUPS
Waiter Portal (Shared Device)
text
┌─────────────────────────────┐
│  SELECT WALTER              │
│  ↓ Dropdown: Abebe ▼        │
...
┌─────────────────────────────┐
│  ABEBA'S TABLES             │
│  Table 3: 2 items [PAID ✓]  │
│  Table 5: 1 coffee [PENDING]│
│  Table 7: FREE              │
│  [ + NEW ORDER ] [LOGOUT ]  │
└─────────────────────────────┘
Manager Assignment
...
│  Assign: [Abebe ▼] to □    │
└─────────────────────────────┘
✅ PERMISSIONS UPDATE (Add to rbac-permissions-guide.md)
text
WAITER (Shared Portal Mode):
✅ waiter.use_shared_portal          // Select name on shared device
✅ waiter.view_assigned_tables       // See only assigned tables
✅ waiter.create_orders_assigned     // Orders for assigned tables only
✅ waiter.process_payments_limited   // Max 15% discount
❌ payments.refund                  // Manager PIN required
❌ payments.void                    // Manager PIN required

BRANCH_MANAGER:
✅ manager.assign_tables             // Assign tables to waiters
✅ manager.view_all_waiter_sessions  // Monitor who's working
✅ manager.override_waiter_pin       // Discounts/voids/refunds
🚀 IMPLEMENTATION PRIORITY
text
Week 1: 
✅ Database tables (table_assignments, waiter_sessions)
✅ Waiter session APIs
✅ Basic table assignment UI (manager)

Week 2:
✅ Order creation with waiter tracking
✅ Waiter portal UI (shared device)
✅ Session timeout (5 min idle)

Week 3:
✅ Manager table assignment UI (drag-drop)
✅ Waiter performance reports
