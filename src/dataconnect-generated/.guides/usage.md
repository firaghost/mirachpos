...
```ts
import { useCreateInventoryItem, useListMenuItems, useUpdateMenuItemInStock, useListOrdersForUser } from '@dataconnect/generated/react';
// The types of these hooks are available in react/index.d.ts

const { data, isPending, isSuccess, isError, error } = useCreateInventoryItem(createInventoryItemVars);

const { data, isPending, isSuccess, isError, error } = useListMenuItems();

const { data, isPending, isSuccess, isError, error } = useUpdateMenuItemInStock(updateMenuItemInStockVars);

const { data, isPending, isSuccess, isError, error } = useListOrdersForUser(listOrdersForUserVars);
