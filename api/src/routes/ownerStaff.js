    ...
    const ensureDefaultRoles = async (trx, tenantId) => {
      ...
      for (const x of existing) {
        ...
        if (existingByName.has(key)) {
          duplicateIdsToDelete.push(String(x.id));
        } else {
          existingByName.set(key, x);
        }
      }
      if (duplicateIdsToDelete.length > 0) {
        await trx.from('roles').where({ tenant_id: tenantId }).whereIn('id', duplicateIdsToDelete).delete();
      }
    ...
    const makeOwnerStaffRouter = () => {
      ...
      const requireOwner = (req, res) => {
        try {
          ...
          if (role !== 'Cafe Owner' && role !== 'Branch Manager') {
            res.status(403).json({ error: 'forbidden' });
            return false;
          }
          return true;
