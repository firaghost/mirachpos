...
const {
  ...
  validateSuperadminSmsTest,
} = require('../middleware/validators');
const { makeSuperadminTenantsRouter } = require('./superadmin/tenants');
const { makeSuperadminTaxRouter } = require('./superadmin/tax');
const { makeSuperadminInvoicesRouter } = require('./superadmin/invoices');
const { makeSuperadminIntegrationsRouter } = require('./superadmin/integrations');
...
const makeSuperadminRouter = () => {
  ...
  r.use(makeSuperadminTenantsRouter());
  r.use(makeSuperadminTaxRouter());
  r.use(makeSuperadminInvoicesRouter());
  r.use(makeSuperadminIntegrationsRouter());
  ...
  r.get('/superadmin/overview', requireSuperadmin, validateSuperadminOverviewQuery, async (_req, res, next) => {
    try {
      ...
      const errorRateDelta = errorRate5xx - jobsPrev > 0 ? jobsFailPrev / jobsPrev : 0;
  ...
  r.post('/superadmin/sms/test', requireSuperadmin, validateSuperadminSmsTest, async (req, res, next) => {
    try {
      const { to, message } = req.validatedBody || req.body;
      const cfg = await getPlatformPaymentConfig();

      const smsCfg = cfg?.sms || { enabled: false };
      if (!smsCfg.enabled) {
        return res.status(409).json({ error: 'sms_disabled' });
      }
